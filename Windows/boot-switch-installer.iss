...
[Run]
; 1) Create a scheduled task "BootSwitch" that runs boot-switch.exe as SYSTEM
...
Filename: "{sys}\schtasks.exe"; \
    ...
    Flags: runhidden; \
    Description: "Create BootSwitch scheduled task";

; 2) Fix power conditions: allow on battery, don't stop on battery
Filename: "{sys}\WindowsPowerShell\v1.0\powershell.exe"; \
    Parameters: "-NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -Command ""$name = 'BootSwitch'; $t = Get-ScheduledTask -TaskName $name -ErrorAction SilentlyContinue; if ($t) {{ $s = $t.Settings; $s.DisallowStartIfOnBatteries = $false; $s.StopIfGoingOnBatteries = $false; Set-ScheduledTask -TaskName $t.TaskName -TaskPath $t.TaskPath -Settings $s }}"""; \
    Flags: runhidden; \
    Description: "Adjust BootSwitch task power settings";

; 3) Run the task once immediately so you don't need to reboot to test
Filename: "{sys}\schtasks.exe"; \
    Parameters: "/Run /TN ""BootSwitch"""; \
    Flags: runhidden; \
    Description: "Run BootSwitch scheduled task once";
